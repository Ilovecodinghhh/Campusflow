...
现有 Demo 是静态前端页面，`apps/web/app.js` 承担了页面状态、交互和渲染；后端 `apps/api/campusflow/engine.py` 已经抽出了可测试的意图解析、推荐和风险预审逻辑，`service.py` 负责业务编排与审计。这说明方案已经具备前后端解耦意识，并已形成初步 API 契约。
...
| `POST /api/applications/submit` | 提交申请并写入审计 | 绕过审批的直接写入 |
